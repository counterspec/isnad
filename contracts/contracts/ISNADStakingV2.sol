...
/**
 * @title ISNADStakingV2
 * @notice Manages attestations (stakes) on ISNAD resources
 * @dev Compatible with any ERC20 token (including Clawnch/Clanker tokens)
 * 
 * V2 Changes:
 * - Slashing transfers tokens to burn address instead of calling burnFrom()
 * - Works with any standard ERC20, no BURNER_ROLE required
 * 
 ...
 */
contract ISNADStakingV2 is ReentrancyGuard, AutoUnpausable {
    using SafeERC20 for IERC20;

    bytes32 public constant ORACLE_ROLE = keccak256("ORACLE_ROLE");

    // Standard burn address (tokens sent here are effectively burned)
    ...
    uint256 public totalSlashed; // Track total tokens sent to burn address
     ...
     * @dev Sends tokens to burn address (0x...dEaD) - compatible with any ERC20
     */
    function slash(bytes32 attestationId) external onlyRole(ORACLE_ROLE) nonReentrant {
        ...
        // Send tokens to burn address (works with any ERC20)
