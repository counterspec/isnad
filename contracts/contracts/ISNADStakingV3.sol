...
/**
 * @title ISNADStakingV3
 * @notice Manages attestations (stakes) on ISNAD resources
 * @dev V3: Governance-settable MAX_STAKE_PER_AUDITOR
 * 
 * Changes from V1:
 * - MAX_STAKE_PER_AUDITOR is now a state variable, not a constant
 * - setMaxStakePerAuditor() restricted to DEFAULT_ADMIN_ROLE (Timelock)
 * - Bounds: min 1K, max 1M tokens
 * 
 ...
 */
contract ISNADStakingV3 is ReentrancyGuard, AutoUnpausable {
    ...
    // Whale caps - V3: now governance-settable
    uint256 public maxStakePerAuditor;
    uint256 public constant MAX_STAKE_PERCENT = 33; // 33% of total stake on resource

    // Bounds for maxStakePerAuditor (governance can't set outside these)
    uint256 public constant MIN_MAX_STAKE = 1_000 * 1e18;      // Floor: 1K tokens
    uint256 public constant MAX_MAX_STAKE = 1_000_000 * 1e18;  // Ceiling: 1M tokens
    ...
    event MaxStakePerAuditorUpdated(
        uint256 oldValue,
        uint256 newValue
    );
     ...
     * @param _maxStakePerAuditor Initial max stake per auditor
     */
    constructor(address _token, address admin, uint256 _maxStakePerAuditor) {
        require(_token != address(0), "Invalid token address");
        require(admin != address(0), "Invalid admin address");
        require(_maxStakePerAuditor >= MIN_MAX_STAKE, "Below minimum");
        require(_maxStakePerAuditor <= MAX_MAX_STAKE, "Above maximum");
        
        token = IERC20(_token);
        maxStakePerAuditor = _maxStakePerAuditor;
    ...
    /**
     * @notice Update the maximum stake per auditor (governance only)
     * @param newMax New maximum value
     * @dev Restricted to DEFAULT_ADMIN_ROLE (should be Timelock)
     */
    function setMaxStakePerAuditor(uint256 newMax) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(newMax >= MIN_MAX_STAKE, "Below minimum (1K)");
        require(newMax <= MAX_MAX_STAKE, "Above maximum (1M)");
        
        uint256 oldValue = maxStakePerAuditor;
        maxStakePerAuditor = newMax;
        
        emit MaxStakePerAuditorUpdated(oldValue, newMax);
    }
    ...
    function stake(
        ...
        // Check whale caps (using state variable now)
        require(
            auditorTotalStake[msg.sender] + amount <= maxStakePerAuditor,
