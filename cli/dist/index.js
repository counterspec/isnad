...
function resolveNetwork(opts) {
    if (opts.testnet)
        return 'sepolia';
    if (opts.network === 'sepolia' || opts.network === 'testnet')
        return 'sepolia';
    if (opts.network === 'mainnet' || opts.network === 'base')
        return 'mainnet';
    // Check env var
...
program
    ...
    try {
        ...
        console.log('\n✅ Faucet successful!');
        console.log(`New balance: ${(0, viem_1.formatUnits)(balance, 18)} tISNAD`);
    }
    catch (error) {
        console.error('Error:', error.message);
        process.exit(1);
...
program
    ...
    if (cmdOptions.file) {
        ...
        content = (0, viem_1.toBytes)(cmdOptions.string);
    }
    else {
...
program
    ...
    try {
        ...
        console.log(`Balance: ${(0, viem_1.formatUnits)(balance, 18)} ${network === 'sepolia' ? 'tISNAD' : 'ISNAD'}`);
    }
    catch (error) {
        console.error('Error:', error.message);
        process.exit(1);
